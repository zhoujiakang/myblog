...
客户端默认打开“随心记”。点击“记录随心记”即可写下摘录、想法或问题；这些内容自动保存在博客目录的 `.blog-studio/` 中，仅留在本机，不会上传到 GitHub。

在左侧勾选一条或多条随心记，点击“AI 整理为草稿”。第一次使用时，先点击右上角的星光图标，输入你自己的 DeepSeek API Key。Key 只保存到这台 Mac 的应用配置中，永远不会写入博客目录或 Git 仓库。

AI 生成的内容会创建为 Hexo 草稿并自动进入文章编辑器。修改确认后，再将文章状态改为“已发布”并发布到 GitHub。
...
浏览器打开 <http://127.0.0.1:4180>。客户端仅在这台电脑上运行，不会公开到互联网。

在客户端中可以新建文章、保存草稿、选择分类和标签、设置封面、上传正文图片，并实时预览 Markdown。写完后把状态改成“已发布”，点击右上角“发布到 GitHub”，网站会在几分钟内自动更新。

关闭客户端时，在运行它的终端按 `Control + C`。
